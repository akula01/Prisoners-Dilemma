		...
		for (R = 1; R <= Parameters.numRuns; R++){
			...
			for (G=0; G<Parameters.generations; G++){
				...
				for (int i=0; i<Parameters.popSize; i++){
					...
					int numOpponents = Parameters.popSize / 10;

					//  Set gene values to a randum sequence of 1's and 0's
					int [] popSample = new int [Parameters.popSize];

					for (int p=0; p<Parameters.popSize; p++) popSample[p] = p;

					// shuffle the array
					for (int p=0; p<1000; p++){
						int i1 = Search.r.nextInt(Parameters.popSize);
						int i2 = Search.r.nextInt(Parameters.popSize);

						// swap the indices
						int temp = popSample[i1];
						popSample[i1] = popSample[i2];
						popSample[i2] = temp;
					}

					int countedOpponents = 0;
					int index = 0;

					Chromo [] opponents = new Chromo[numOpponents];

					while (countedOpponents < numOpponents){
						int opponentIndex = popSample[index];

						if(opponentIndex != i){
							opponents[countedOpponents] = member[opponentIndex];
							countedOpponents++;
						}
					...
					problem.doRawFitnessTest(member[i], opponents, 100);
